...
@callback(Output("reset-modal", "is_open"), Input("reset-account-button", "n_clicks"), State("reset-modal", "is_open"), prevent_initial_call=True)
def toggle_reset_modal(n, is_open):
    if n: return not is_open
    return is_open

@callback(
    [Output("wallet-balance-store", "data", allow_duplicate=True), Output("portfolio-store", "data", allow_duplicate=True), Output("trading-history-store", "data", allow_duplicate=True), Output("wallet-history-store", "data", allow_duplicate=True), Output("reset-modal", "is_open", allow_duplicate=True), Output("reset-alert-placeholder", "children")],
...
@callback(Output("reset-modal", "is_open", allow_duplicate=True), Input("cancel-reset-button", "n_clicks"), State("reset-modal", "is_open"), prevent_initial_call=True)
