...
from utils.data_handler import (fetch_stock_data, get_key_metrics, calculate_technical_indicators, fetch_news, fetch_corporate_actions)
...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(dbc.Input(id="stock-ticker-input", placeholder="Enter Stock Ticker (e.g., AAPL, RELIANCE.NS)", type="text"), width=8),
        dbc.Col(dbc.Button("Analyze Stock", id="analyze-button", color="primary", className="w-100"), width=4)
    ], className="mb-4"),
...
def update_dashboard(n_clicks, ticker):
    ...
    news, dividends, splits = fetch_news(ticker.upper()), *fetch_corporate_actions(ticker.upper())
    ...
    news_content = [dbc.ListGroupItem([html.H6(html.A(a['title'], href=a.get('link'), target='_blank')), html.P(f"Publisher: {a.get('publisher', 'N/A')}")]) for a in news if 'link' in a]
    
    actions_content = []
    if dividends is not None and not dividends.empty:
        actions_content.extend([html.H5("Recent Dividends"), dbc.Table.from_dataframe(dividends, dark=True)])
    if splits is not None and not splits.empty:
        actions_content.extend([html.H5("Recent Stock Splits"), dbc.Table.from_dataframe(splits, dark=True)])
    if not actions_content: actions_content.append(dbc.Alert("No recent actions.", color="info"))
...
@callback([Output("transaction-modal", "is_open"), Output("modal-title", "children"), Output("confirm-transaction-button", "color"), Output("confirm-transaction-button", "custom_data")], [Input("buy-button", "n_clicks"), Input("sell-button", "n_clicks")], prevent_initial_call=True)
...
def execute_transaction(n, type, qty, ticker, balance, portfolio, trade_hist, wallet_hist):
    if not all([type, qty, ticker]): return (dash.no_update,) * 4 + (dbc.Alert("Invalid details.", color="warning"), False)
    try:
        ...
        if type == "BUY":
            ...
            msg, color = f"Purchased {qty} of {ticker}.", "success"
        elif type == "SELL":
            ...
            if profit > 0:
                fee = profit * 0.10
                balance -= fee
                wallet_hist.append({'Date': pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"), 'Description': f"Fee on {ticker} profit", 'Amount': f"-₹{fee:,.2f}", 'Balance': f"₹{balance:,.2f}"})
                msg = f"Sold {qty} of {ticker} for a profit of ₹{profit:,.2f}. Fee of ₹{fee:,.2f} applied."
            else:
                msg = f"Sold {qty} of {ticker}."
            portfolio[ticker]['quantity'] -= qty
            if portfolio[ticker]['quantity'] == 0: del portfolio[ticker]
            color = "success"
        trade_hist.append({'Date': pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"), 'Stock': ticker, 'Type': type, 'Quantity': qty, 'Price': f"₹{price:,.2f}", 'Total': f"₹{value:,.2f}"})
        wallet_hist.append({'Date': pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"), 'Description': f"{type} {ticker}", 'Amount': f"-₹{value:,.2f}" if type == "BUY" else f"+₹{value:,.2f}", 'Balance': f"₹{balance:,.2f}"})
...
@callback([Output("watchlist-store", "data"), Output("watchlist-alert-placeholder", "children"), Output("watchlist-button", "children"), Output("watchlist-button", "outline")], Input("watchlist-button", "n_clicks"), [State("current-ticker-store", "data"), State("watchlist-store", "data")], prevent_initial_call=True)
...
@callback([Output("alert-modal", "is_open"), Output("upper-alert-input", "value"), Output("lower-alert-input", "value")], Input("alert-button", "n_clicks"), [State("current-ticker-store", "data"), State("price-alert-store", "data")], prevent_initial_call=True)
def open_alert_modal(n, ticker, alerts):
    if not ticker: return False, None, None
    a = alerts.get(ticker, {})
    return True, a.get('upper'), a.get('lower')

@callback([Output("price-alert-store", "data"), Output("alert-modal", "is_open", allow_duplicate=True), Output("transaction-alert-placeholder", "children", allow_duplicate=True)], Input("confirm-alert-button", "n_clicks"), [State("upper-alert-input", "value"), State("lower-alert-input", "value"), State("current-ticker-store", "data"), State("price-alert-store", "data")], prevent_initial_call=True)
